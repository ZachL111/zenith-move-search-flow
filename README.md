...
zenith-move-search-flow is a Elixir project for chess and game engines. It focuses on this technical goal: Build an Elixir toolkit that studies search behavior through append-only fixtures, with checkpoint recovery checks and bounded memory input sets.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 153, risk penalty 5, latency penalty 4, and weight bonus 5. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
